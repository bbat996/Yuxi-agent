...
class AgentCreateRequest(BaseModel):
    ...
    agent_type: str = Field(default="custom", description="智能体类型")
    avatar: Optional[str] = Field(None, description="头像URL")
    system_prompt: Optional[str] = Field(None, description="系统提示词")
    model_config: Optional[Dict[str, Any]] = Field(default_factory=dict, description="模型配置")
    tools_config: Optional[Dict[str, Any]] = Field(default_factory=dict, description="工具配置")
    knowledge_config: Optional[Dict[str, Any]] = Field(default_factory=dict, description="知识库配置")
...
class AgentUpdateRequest(BaseModel):
    ...
    knowledge_config: Optional[Dict[str, Any]] = Field(None, description="知识库配置")
    tags: Optional[List[str]] = Field(None, description="标签列表")
    is_public: Optional[bool] = Field(None, description="是否公开")
    is_active: Optional[bool] = Field(None, description="是否激活")

class AgentConfigOverride(BaseModel):
    config_override: Dict[str, Any] = Field(..., description="配置覆盖")
    ...
    try:
        with db_manager.get_session_context() as session:
            ...
            if search:
                ...
                query = query.filter(
                    or_(
                        CustomAgent.name.like(search_pattern),
                        CustomAgent.description.like(search_pattern)
                    )
                )
            ...
            for agent in agents:
                ...
                # 获取用户的实例信息（如果存在）
                instance = session.query(AgentInstance).filter(
                    and_(
...
async def duplicate_agent(
    ...
    try:
        with db_manager.get_session_context() as session:
            # 查询原智能体
            original_agent = session.query(CustomAgent).filter(
                and_(
                    CustomAgent.agent_id == agent_id,
                    CustomAgent.deleted_at.is_(None)
                )
...
async def create_agent_instance(
    agent_id: str,
    config_override: AgentConfigOverride = Body(...),
    current_user: User = Depends(get_required_user)
):
    """为用户创建智能体实例"""
    try:
        with db_manager.get_session_context() as session:
            ...
            # 检查是否已有实例
            existing_instance = session.query(AgentInstance).filter(
                and_(
                    AgentInstance.agent_id == agent_id,
                    AgentInstance.user_id == current_user.id
                )
            ...
            if existing_instance:
                # 更新现有实例
                existing_instance.config_override = config_override.config_override
