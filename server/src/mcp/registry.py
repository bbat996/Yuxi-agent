...
class MCPSkillRegistry:
    ...
    async def refresh_skills(self, force: bool = False):
        ...
        try:
            with self.db_manager.get_session_context() as session:
                # 获取所有激活的MCP技能
                skills = session.query(MCPSkill).filter(MCPSkill.is_active == True).all()
    ...
    async def _update_skill_stats(self, skill_id: str, start_time: datetime, success: bool):
        ...
        try:
            ...
            with self.db_manager.get_session_context() as session:
                skill = session.query(MCPSkill).filter(MCPSkill.skill_id == skill_id).first()
    ...
    async def test_skill(self, skill_id: str) -> Dict[str, Any]:
        ...
        client = await self.get_client(skill_id)
        if not client:
            return {"success": False, "error": "技能不可用", "timestamp": datetime.now().isoformat()}
        ...
        try:
            ...
            if not is_connected:
                return {"success": False, "error": "连接失败", "timestamp": datetime.now().isoformat()}
